...
if (isset($_GET['subject_name'])) {
	$subject = sanitizeMySQL($conn, $_GET['subject_name']);
	...
	if (!$result) {
			...
			if ($rows) {
				echo "<h2>Search term: ".$_GET['subject_name']."</h2>
				<table class=center><tr><th>Tutor</th><th>School</th>";
				while ($row = $result->fetch_assoc()) {
					echo "<tr>";
	echo "<td><a href=profile.php?tutor_id=".$row["tutor_id"].">".$row["firstname"]." ".$row["lastname"]."</td><td>".$row["school_name"]."</a></td>";					echo "</tr>";
				}
				echo "</table>";
			} else {
				echo "<p>No results!</p>";
